...
public class ETLWikiEJB {
	...
	/**
	 * Obtiene los datos de las p�ginas en la que se realiz� cambios recientes
	 * 
	 * @param listaCambios
	...
	public void obtenerDatosPage(List<RecentChanges> listaCambios) {
		...
		for (RecentChanges recentChanges : listaCambios) {
			...
			// Si el id de la p�gina es diferente de -1 quiere decir que la
			// p�gina existe, de lo contrario fue eliminada
			if (idPagina != -1) {
				...
				pagina.setText("La p�gina fue eliminada");
				pagina.setPageId(-1);
