...
public class ControladorTipoUsuario implements Serializable {
		...
		if (!validarCamposVacios()) {
			...
			try {
				...
				try {
					...
					auditoriaEJB.crearAuditoria("AuditoriaTiposUsuarios", accion, "TU creado: " + tu.getNombre(),
	...
	public void eliminarTipo(TipoUsuario tu) {
		try {			
			...
			String browserDetail = Faces.getRequest().getHeader("User-Agent");
			auditoriaEJB.crearAuditoria("AuditoriaTiposUsuarios", accion, "TU eliminado: " + tu.getNombre(),
					usuario.getNombre(), browserDetail);
	...
	public void editarTipo() {
		if (!validarCamposVacios()) {
			...
			try {
				...
				String browserDetail = Faces.getRequest().getHeader("User-Agent");
				auditoriaEJB.crearAuditoria("AuditoriaTiposUsuarios", accion, "TU editado: " + nombre,
						usuario.getNombre(), browserDetail);
